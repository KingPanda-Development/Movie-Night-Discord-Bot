...
class Cluster extends events_1.default {
    ...
    _handleError(error) {
        ...
        const match = stackTrace.match(/\/Movie-Night-Discord-Bot\/([^/]+)\/([^/]+)\/([^/]+\.js):(\d+):(\d+)/) || stackTrace.match(/\/Movie-Night-Discord-Bot\/([^/]+)\/([^/]+\.js):(\d+):(\d+)/) || stackTrace.match(/\/Movie-Night-Discord-Bot\/([^/]+\.js):(\d+):(\d+)/);
        let fileName;
        let lineNumber;
        let columnNumber;
        ...
        if (match && !match[0].includes('node_modules')) {
            console.log(error)
            console.error(c.gray("#" + lineNumber + ":" + columnNumber) + `〔 ${capitalizeFirstLetterOnly(fileName)} 〕➜ Error Occurred:`, c.red(error.message));
        } else {
