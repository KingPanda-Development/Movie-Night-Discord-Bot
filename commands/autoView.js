...
module.exports = {
	...
	async execute(interaction, settings) {
		...
		if (option == "showsetting") return interaction.reply({ content: `Currently setting is: ${settings.autoViewed ? "on" : "off"}`, ephemeral: true });

		await Setting.updateOne({ guildID: interaction.guild.id }, { autoViewed });

		return interaction.reply({ content: `Auto view has now been set to: ${autoViewed ? "on" : "off"}`, ephemeral: true });
	},
};
