...
module.exports = {
	...
	async execute(interaction, settings) {
		...
		if (interaction.options.getSubcommand() == SHOW_SETTING) return interaction.reply({ content: `Current role is ${currentRole == null ? "admins only" : currentRole == "all" ? "allow everyone." : "<@&" + currentRole + ">"}`, ephemeral: true });

		await Setting.updateOne({ guildID: interaction.guild.id }, { deleteMoviesRole });
		
		return interaction.reply({ content:
														deleteMoviesRole ? deleteMoviesRole == "all" ? "All users will now be able to delete movies." : `Users with administrator or the role <@&${deleteMoviesRole}> will now be able to delete movies.` : "Only Admins will be able to delete movies.",
														ephemeral: true
														 });
	},
};
