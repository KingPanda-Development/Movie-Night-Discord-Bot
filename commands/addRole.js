...
module.exports = {
	...
	async execute(interaction, settings) {
		...
		if (interaction.options.getSubcommand() == SHOW_SETTING) return interaction.reply({ content: `Current role is ${currentRole == null ? "allowing everyone to add movies." : "<@&" + currentRole + ">"}`, ephemeral: true });
		...
		return interaction.reply({ content: addMoviesRole ? `Users with administrator or the role <@&${addMoviesRole}> will now be able to add movies.` : "Setting for role allowed to add movies has been cleared. Anyone will be able to add movies now.", ephemeral: true });
	},
};
